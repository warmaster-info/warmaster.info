...
artillery units and machines will be described in their
particular army list.
...
Machines are more complex than artillery and some
have their own rules for fighting in combat, as described
in their particular army list.
...
their own, as described in their individual army list.
...
image::artillery-and-machines/trolls.webp[width=1671,height=480,role=column-span]
...
image::artillery-and-machines/cannon-shot.svg[width=1300,height=851,id=cannon-shot]
...
image::artillery-and-machines/cannon.webp[width=993,height=581,role=decorative]
...
image::artillery-and-machines/elven-bolt-thrower.webp[width=595,height=561,role=decorative]
...
up to two stands so long as they are touching and along the
missile’s path.

The target unit suffers two attacks plus +1 if an additional
stand from the same unit is within the missile’s path. A
shot that skewers into a different unit inflicts one attack
...
touching another enemy unit. Any hits on engaged units from
either side will carry
forward into the combat round and count as having
...
takes this into account. Units that are not engaged [.edit]#in combat# are
