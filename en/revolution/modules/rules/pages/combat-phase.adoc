...
include::partial$diagrams-incomplete.adoc[]
...
by using their initiative. Units using their initiative must
always move against the nearest enemy they can see.
Units acting on an order can move against any enemy
within reach and which they can see. In both cases, the
...
be arranged as shown in Diagram 34.1.

.Top: Two infantry units in combat | Bottom: A unit of cavalry in combat with a unit of infantry.
image::combat-phase/combat-engagement.png[]
...
.Top: Two infantry units in combat | Bottom: A unit of cavalry in combat with a unit of infantry.
image::combat-phase/interlinked-engagement.png[]
...
move against enemy units. Where a charge is intended,
...
image::combat-phase/orc-boss.png[]
...
target then it cannot charge it. Note that because units
...
A stand can ‘see‘ if it is able to draw an
uninterrupted line of sight from its front
edge to any part of the enemy unit as
described in the Shooting section (p.27).
All intervening stands, friend or foe, block
a stand‘s line of sight. All intervening
terrain also blocks the line of sight with the
exception of low obstacles, rivers/streams,
marshes, and similarly low features.
Character stands never block a unit‘s line of
sight, as explained in the Generals, Wizards
and Heroes section (p.59).
...
checked using a simple guide if necessary. See Diagrams
35.1 and 35.2.

// TODO: diagram text
image::combat-phase/zone-infantry.png[]

image::combat-phase/zone-cavalry.png[]
...
rear it charges in the rear. See Diagram 36.1.
...
in the fornt. If a unit is positioned in such a way that it
is exactly divided between two zones then roll a dice
to decide - eg. 1, 2, or 3 the front; 4, 5, or 6 the side. See
Diagram 36.2.

image::combat-phase/chaos-knights.png[]
...
image::combat-phase/charge-move-1.png[]

image::combat-phase/charge-move-2.png[]
...
image::combat-phase/chaos-knight-hero.png[]
...
line if they can do so.

// Moved slightly earlier
// Spelling against
.The charging stand is placed against the closest enemy.
image::combat-phase/maximise-frontage-1.png[]

Stands are placed into a coherent formation as they move.
Place a second stand beside the first, and place the third
...
// Moved slightly earlier
.Remaining stands are placed one at a time to make a battle line - the stands must be placed as shown because this is the only way they can maximise their frontage.
image::combat-phase/maximise-frontage-2.png[]
...
image::combat-phase/dwarf-warriors.png[]
...
The first charging stand is always positioned centre-to
centre with the closest enemy stand where possible.
If this is not possible then the first charging stand is
positioned as closely to centre-to-centre as it can go, but
there must be at least 1 full cm of exposed stand edge to
contact against.

image::combat-phase/dwarf-hero.png[]
...
// TODO diagram
.There must be at least 1cm of edge for the charging stand to contact.
image::combat-phase/charge-blocked.png[]
...
then the move is not allowed. See Diagram 38.1.
...
// TODO diagram
.If a unit has insufficient move to form a battle line stands can be placed behind or staggered back.
image::combat-phase/unable-to-form-a-battle-line.png[]
...
than 1cm of exposed edge avilable to contact.
...
.If the closest visible stand’s indicated edge is covered consider the next closest stand. | This cavalry stand cannot charge to the front of the closest visible enemy stand because of a friendly infantry stand engaged in combat. | The charger is placed against the next closest stand.
image::combat-phase/closest-visible-stand.png[]
...
units then the unit is described as ‘pinned‘. In the case
...
charge edge-to-edge. See Diagram 38.3.

image::combat-phase/pinned-target.png[]
...
.The first stand is positioned as usual. Note that the remaining stands maximise contact because they have enemy stands to their front.
image::combat-phase/irregular-formations.png[]
...
.1. Unit charges enemy infantry in the side. Form a battle line as usual. | 2. Charging unit overlaps using stands that are not touching. | Note that an envelopment can potentially bring charging stands into the firing arc of enemy stands that have not already shot at the charger.
image::combat-phase/enveloping-charge.png[]

image::combat-phase/battle-view.png[]

== Attacks

Regardless of whose turn it is, all units which are
engaged in combat can fight in the Combat phase. Work
...
those fighting (see Advance, p.50).
...
.Where a stand is touching two or more enemy units (as with Stand B) the player can decide which to attack.
image::combat-phase/stands-touching.png[]
...
image::combat-phase/dwarf-slayer-hero.png[]
...
image::combat-phase/dark-elves.png[]
...
image::combat-phase/harpy-hero.png[]
...
terrain. See the section on Confusion, p.55.
...
needed to hit will be 5+ and 6 respectively. See Defended
& Fortified Troops (p.53).
...
as described in the Shooting phase section (p.28). This
...
image::combat-phase/wolf-rider.png[]

.Combat results
****
--
After units have attacked it is time to work out the result. If one side is destroyed then the combat is automatically
won by the survivor, otherwise compare the number of hits inflicted by both sides. In cases where a unit has been
...
the combat is over. See Fall Backs (p.51) for more details.

The exception to the fall back rule is that defended or fortified units can stand their ground instead of
falling back. Players may decide to fall back with such units if they prefer. See Defended and Fortified
Troops (p.53) for rules about fighting in defended and fortified positions.

== One Side Scores More Hits – Win!

If one side scores more hits than the other, it wins the combat round. The loser must retreat 1cm for
each hit taken in excess of hits inflicted. So if one side scores three hits and the other scores one hit,
the second side loses by two and must retreat 2cm. See Retreats (p.45) for further details about how
to move retreating units.

If a unit wins, the player has a choice of what to do next. The victorious unit can fall back, stand its
ground or pursue. If a unit destroys its enemy, it cannot pursue but can advance instead.

*Fall Back.* The victorious unit falls back up to 3D6cm. The combat is over.
...
in the same way as before. See Pursuit (p.47). Any hits already scored are carried over into the next
...
existing one. See Enemy Destroyed (p.50).
--
****
...
and the unit it belongs to is not confused. The stand
...
from a completely different unit. See Diagram 44.1.

// TODO lots of text on this diagram
.TODO In this example, all supporting stands are marked with an asterisk. Notice how unit A has both a stand fighting and a stand supporting.
image::combat-phase/supporting-troops.png[]
...
During the Command phase, a unit is allowed to make
a supporting charge if it is positioned behind another
so that at least one stand could theoretically support in
combat from the rear.
...
image::combat-phase/sword-banner.png[]
...
into touch with enemies - neither into edge contact nor
...
Round any fractions up to the nearest cm - the minimum
...
retreating player can decide which order to move the
units in.

.The two stands are both touching the enemy - the player who has lost the combat can decide which of the stands to use to determine the retreat.
image::combat-phase/retreat-1.png[]
...
// TODO diagram
.In the two examples, the defeated units retreat directly away from their enemy - in straghtforward cases such as these, the direction of retreat is easily determined.
image::combat-phase/retreat-2.png[]
...
of touching enemy stands. If this is equal, the retreating
player can nominate which of the possible directions it
will retreat. For the purpose of calculating the retreat
direction, enemy stands touching corner-to-corner count
...
following: enemy stands, stands of other friendly units
that are engaged in combat (including friendly units that
have already retreated from the same combat during
that round), terrain that is impassable, and stands from
unengaged friendly units that do not make way as noted
above.
...
// TODO lots of text diagram
.Here is a blocked retreat in two stages. On the left, the position before retreats with the retreat paths indicated for each retreating unit. The right hand example shows a retreat of 4cm. The rear defeated unit moves to the side and away from its contacted edge - creating space for one stand to move back. In practice, a retreat of such a large distance would result in causalities that would complicate things - but we‘ve left all stands in place to illustrate the process.
image::combat-phase/retreat-blocked.png[]
...
for further rules for confused units and making way
(p.56).
...
In rare occasions a unit of three stands may be split
...
automatically destroyed. Owning player decides which
one.
...
.These stands slide along the enemy line, contacting the uppermost edge.
image::combat-phase/retreat-incidental-contact.png[]
...
Move victorious units after enemy units have retreated
after the losing side has completed any necessary moves
to make way and remove any stands whose retreat is
blocked.

image::combat-phase/skeleton-infantry.png[]
...
. Infantry units cannot pursue cavalry or chariots
  ...
  of friends in some situations. There are a few
  ...
  For further exceptions see Infantry contact with
  cavalry on p.49.
...
. Non-flying units cannot pursue flying units
  retreating from the same engagement. Exceptions;
  ...
  further exceptions see Infantry contact with
  cavalry on p.49.
****
...
to move into support as described under Supporting
Pursuits p.48.
...
instead (See Supporting Pursuits p.48).
...
image::combat-phase/punk-orc.png[]
...
// TODO text
.This diagram shows a typical direct pursuit move where two stands have pursued straight forward and the third has been placed back into formation. Note that the third pursuing stand could be placed anywhere the player wants - the position shown is merely one option.
image::combat-phase/pursuit-direct.png[]
...
// TODO text
.An indirect pursuit. No pursuer can move ‚straight forward‘ so the first pursuing stand is moved as for a charge. In this case, the pursuing unit lies to the defeated unit‘s side so the first pursuer moves to the flank as shown. The second pursuer can then move into formation as the player wishes - the position shown is merely one example.
image::combat-phase/pursuit-indirect.png[]
...
.Supporting pursuit. The second victorious unit cannot see an enemy but can pursue into support. Notice that the right victorious stand touch a losing unit corner-to-corner and therefore fights in the following round.
image::combat-phase/pursuit-supporting.png[]
...
into corner-tocorner contact with enemy, in which
...
// TODO diagram
.In this example, the victorious unit pursues directly forward with the two right enemy stands - the right most of which ‘slides‘ against the right enemy cavalry unit. The left most victorious stand could be placed in formation anywhere - it has been positioned as shown to demonstrate how incidental contact can be used to deliberately bring other units into an engagement.
image::combat-phase/incidental-contact.png[]
...
// TODO diagram
.In this situation, the pursuing infantry maintain contact with the enemy cavalry because they are touching corner-to-corner.
image::combat-phase/pursuit-maintain-contact.png[]
...
image::combat-phase/goblin-wolves.png[]
...
This also applies on the supporting unit.

image::combat-phase/dwarf-slayers.png[]
...
image::combat-phase/village.png[]
...
automatically confused. See Confusion.
...
with the enemy units. See Diagram 52.1 and 52.2. Other
...
.The unit can fall back anywhere behind the line shown.
image::combat-phase/fall-back-line.png[]

.The unit can fall back any where behind the position shown.
image::combat-phase/fall-back-corner.png[]
...
image::combat-phase/cold-combat.png[]
...
image::combat-phase/medusa-demon.png[]
...
as described in the chapter Siege & Fortresses on p.87.
