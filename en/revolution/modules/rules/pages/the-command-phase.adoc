...
. If the unit you want to move is in dense terrain then
  ...
  buildings, ruins and similar. See the Movement section
  for more about dense terrain (p.22) as well as Advanced
  Terrain Rules for more examples (p.80).
+
[cols="^,^"]
|===
...
in the ensuing Combat phase. See the Combat phase
section for further explanation (p.33).
...
the enemy whilst others do not, as shown in #Diagrams
17.1 to 17.4#. Any units in a brigade that wish to charge do
...
#TODO: Better annotation of diagrams#

.Brigade received an order
image::the-command-phase/charge-from-brigade-1.png[]

image::the-command-phase/charge-from-brigade-2.png[]

.Units C & B can see enemy 2.  Unit B can no longer see enemy 1.  Unit D can see nothing.
image::the-command-phase/charge-from-brigade-3.png[]

.Units B & D can now move, no other charges are possible.  If they do so they must finish their moves touching – as they are moving under brigade orders.
image::the-command-phase/charge-from-brigade-4.png[]
