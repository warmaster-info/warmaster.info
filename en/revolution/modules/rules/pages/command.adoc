[.page=14]
= [.edit]#Command & Initiative#

Units [.bo5a]#and characters are# usually moved in the Command [.edit]#& Initiative# phase. A unit
...
[.bo5a]#Characters move automatically and do so after all unit movement is complete.#

The Command [.edit]#& Initiative# phase proceeds in the following sequence
...
the xref:movement.adoc#movement-distances[Movement section].
...
[.ancients]#During the Initiative sub-phase,#
a unit can use its own initiative to either xref:movement.adoc#moving-chargers[charge] or
xref:movement.adoc#the-path-of-evaders[evade] from the *closest visible enemy unit* within 20cm (see xref:game-values.adoc#visibility-on-the-battlefield[Visibility on the battlefield]).
[.edit]#A unit does not have to use its initiative, it# can instead wait until the
[.edit]#Orders sub-phase#
and attempt to move by means of an order.
The choice is the player’s in most cases; exceptions are
covered in the army lists.
...
player can choose which unit to react to.
...
either *xref:movement.adoc#moving-chargers[charge]* the enemy unit it is reacting to, assuming it
is able to do so, or it can *xref:movement.adoc#the-path-of-evaders[evade]* from the enemy unit it is
...
See xref:movement.adoc#supporting-charges[Supporting charges].
...
////
// Warmaster Original text from Movement chapter:
Units moving by initiative do so during the xref::command.adoc#initiative-movement[Initiative
...
// Todo: merge with the above paragraph.
[.ancients]
...
image::command/initiative-charge-1.webp[width=993,height=682,id=initiative-charge-1]
...
image::command/initiative-charge-2.webp[width=993,height=703,id=initiative-charge-2]
...
[.wmr]
image::command/giant.webp[width=769,height=1176,role=decorative]
...
The player [.bo5a]#first nominates a unit that#
...
received its order and can move full pace (see xref::movement.adoc#movement-distances[Movement [.edit]#distances#]). If the score
is more than the character’s Command value then the
unit has not received its order.  [.edit]#If this is the unit’s first order then# it can move only half
pace. This represents unit’s
commanders being able to act on their own to some
limited extent.
// NB: Half-pace failed orders not in Ancients/Bo5A.

// TODO: Details of a charge move (e.g. visible at start) should be made here.
...
image::command/skinks.webp[width=697,height=579,role=decorative]
...
unit failing to receive a *further* order *cannot* move [.edit]#any further in this phase#.
...
image::command/goblins.webp[width=2070,height=1053,role=column-span]
...
.Command penalties
====
The General has a Command value of 9.  He
...
success! The player moves the cavalry into their new position.

Next, he attempts to give orders to a unit of infantry
10cm away [.wmr]#(no penalty) and within a wood (−1
dense terrain penalty).# Although he only needs 8 or less, this time he
is unlucky and rolls a 10. The unit does not move – the
General has failed an order so the Orders sub-phase comes to an end.
====

.Optional rules
****
*Battle Plans:* All Characters have +1 bonus to their
Command value for their first command of the first
turn. This represents a better state of organisation
and communication before the battle commences.
Note that the Command value can never exceed 10.

*Heavy Cavalry:* a Cavalry unit with 4+ Armour
has an additional −1 penalty for the second and any
subsequent order.
****

image::command/triskelion.webp[width=438,height=457,role=circle]
...
image::command/cavalry-charge.webp[width=1443,height=1161]
...
affect [.edit]#a character’s# ability to move.
...
image::command/brigade-of-three.webp[width=1432,height=501]

.These four units can also be formed into a brigade. Each unit is arranged into a column that is three stands deep.
image::command/brigade-of-four.webp[width=1409,height=680]
****
...
and the
whole brigade moves at half pace or doesn’t move [.edit]#further#
...
[#f-brigade-orders]
image::command/charge-from-brigade-1.svg[width=1355,height=931]

image::command/charge-from-brigade-2.webp[width=1016,height=698]

image::command/charge-from-brigade-3.svg[width=1355,height=931]

image::command/charge-from-brigade-4.svg[width=1355,height=931]
...
image::command/bretonnian-knights.webp[width=1476,height=724,role=column-span]
