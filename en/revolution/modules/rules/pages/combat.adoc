...
.{empty}
image::combat/combat-engagement.svg[width=1312,height=1440,id=f-combat-engagement]
...
image::combat/interlinked-engagement.webp[width=1250,height=918,id=f-interlinked-engagement]
...
image::combat/dwarf-slayer-hero.webp[width=457,height=503,role=decorative]

[.page=40]
[#attacks]
== Attacks

// "can" removed to align with Q&A.
Regardless of whose turn it is, all units which are
engaged in [.qa]#combat fight# in the Combat phase.
...
image::combat/stands-touching.svg[width=1312,height=901,id=stands-touching]
...
enemy are not in the open, i.e. when a stand is fighting
...
At the end of the Combat phase, remaining hits are discounted
from units that are no longer engaged. Hits are only
recorded from one turn to the next for units that remain
engaged in an unresolved combat at the end of the Combat
...
image::combat/wolf-rider.webp[width=993,height=1493,role=decorative]
...
[.qa]#Infantry stands adjacent to fighting infantry units
affect combat results.#
This
...
.{empty}
image::combat/supporting-troops.svg[width=2772,height=1049,id=f-supporting-troops]
...
image::combat/harpy-hero.webp[width=765,height=1069,role=decorative]
...
image::combat/retreat-1.svg[width=1312,height=952,id=retreat-1]
...
image::combat/retreat-2.svg[width=1312,height=916,id=retreat-2]
...
image::combat/fan.webp[width=447,height=455,role=circle]
...
image::combat/retreat-blocked.svg[width=2772,height=765,id=retreat-blocked]
...
image::combat/retreat-incidental-contact.svg[width=1324,height=1028,id=retreat-incidental-contact]
...
image::combat/dark-elves.webp[width=2079,height=623,role=column-span]
...
[.wmr]
image::combat/skeleton-infantry.webp[width=911,height=744,role=decorative]
...
[TIP]
====
The Q&A has a xref:questions-and-answers.adoc#wrapping-pursuit[detailed example with diagrams] showing a full combat sequence including pursuit.
====
...
// NB The whole Pursuit section is significantly edited for Bo5A,
// these edits are (mostly) not included here.
...
// Add "a", "the"
If a pursuing unit cannot see a retreating enemy unit (for
example, because other pursuers have obscured their
view), then the pursuers cannot move into edge contact.
...
image::combat/punk-orc.webp[width=809,height=881,role=decorative]
...
image::combat/pursuit-direct.svg[width=2759,height=921,id=pursuit-direct]
...
image::combat/pursuit-indirect.svg[width=2772,height=975,id=pursuit-indirect]
...
image::combat/incidental-contact.svg[width=1324,height=1528,id=f-incidental-contact]
...
are not permitted to contact any enemy stands edge-to-edge,
not even as incidental contacts. See Supporting
...
No pursuit or charge bonuses apply
...
yet, it becomes part of the current combat. However,
...
infantry pursue enemy infantry and end up in corner-to-corner
or side-to-side edge contact with enemy cavalry or
...
image::combat/pursuit-maintain-contact.svg[width=2772,height=1029,id=pursuit-maintain-contact]
...
See <<incidental-contact>>.

[.page=50]
[#pursuit-in-terrain]
=== Pursuit in terrain

Visibility within woods [.edit]#and villages# is restricted to 2cm for
most purposes (see xref:movement.adoc#terrain[Terrain]), but in the case of combatants we make
...
image::combat/goblin-wolves.webp[width=2089,height=287,role=column-span]
...
an advance to be able to happen. This also applies to
situations when the original engagement is split into
two or more independent combats (either by removal of
stands or different directions of pursuit) until the end of
...
.Optional rule
****
*Forceful Advance:* If the enemy is destroyed in the
first round of combat, the advancing unit gets a
charge bonus as per normal charge.
****

image::combat/dwarf-slayers.webp[width=2200,height=384,role=column-span]
...
combat each side takes it in turn to select and move its
units starting with the player who has the next turn.
...
.{empty}
image::combat/fall-back-line.svg[width=1324,height=920,id=fall-back-line]

.{empty}
image::combat/fall-back-corner.svg[width=1324,height=920,id=fall-back-corner]

image::combat/village.webp[width=2075,height=1049,role=column-span]
...
the victors elect to pursue. Units locked in an unresolved
combat cannot receive orders.
...
// Reason: https://www.specialist-arms.com/forum/index.php?topic=8716.0
...
any hits remaining on units that are no longer
engaged in combat. Note that units that are part of an
unresolved combat do not remove remaining hits but carry
them over into the following turn. This includes any
units that are only supporting.

image::combat/cold-combat.webp[width=2157,height=862]
...
of an advance. Cavalry, chariots, artillery and non-flying
monsters and machines cannot do so. In the case
...
image::combat/medusa-demon.webp[width=1245,height=890,role=decorative]
