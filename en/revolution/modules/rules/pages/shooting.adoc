...
////
// "Range" section moved before target, as shooting range
// is a prerequisite to selecting a target.
// (Same change was made in Bo5A + Ancients.)
////
...
// Q&A: https://warmaster.info/en/revolution/rules/questions-and-answers#obliged-to-shoot
...
// Moved earlier (as in Bo5A) as it's more important
// than the next paragraph which has the individual stand detail.
...
//
// Removed to merge with common Line of Sight rules,
// but following additional rule retained.
////
...
chariots or other enemy for whom the wood is impenetrable.
See xref:movement.adoc#terrain[Terrain].

// Caption added
.Targets in and out of sight.
image::shooting/sight.svg[width=1312,height=785,id=sight]

Some units can see all round – they have 360 degree vision
– and you will find this indicated in the unit’s description
(for example, xref:armies:goblins.adoc#wolf-riders[Wolf Riders]). This means when shooting
...
// edge.
//
// Removed as this is the shooting section.
////

image::shooting/empire-infantry.webp[width=2200,height=322,role=column-span]

// Old heading "How to calculate casualties"
[.page=28]
[.h-bo5a]
...
// Removed "In practise" and "is perfectly acceptable and"
...
[.page=28]
[.h-bo5a]
[#defended-or-fortified-targets]
=== Defended or fortified targets

It is harder to score a hit on a target if it is in a defended
or fortified position.

// Table moved earlier, wording from Bo5A used.

The [.edit]#table# below shows the [.bo5a]#minimum dice roll needed to
score a hit on the target#.

[cols="4,^1",frame=none,grid=rows]
|===
...
//
// Replace with separate paragraphs from Bo5a etc.
////

// This avoids "protected"
[.bo5a]
...
The full definitions and further
rules for troops in defended and fortified positions are
discussed in the xref::combat.adoc#defended-fortified-troops[Combat phase section].

image::shooting/boar-riders.webp[width=2079,height=736,role=decorative]
...
rolled for Armour (one dice per hit) scoring 2, 4, 5 and 6.
As any score of 4 or more equals a ‘save’, 3 hits are
...
formation, never the middle, so the unit retains a viable formation.#
...
over, any remaining hits are discounted. Hits are not recorded
...
image::shooting/wizard.webp[width=688,height=1084,role=decorative]
...
is complete but *before* disregarding remaining hits at the end
of the Shooting phase. The player whose units are being
driven back can decide the order in which drive backs
are resolved.

To determine how far units are driven back, the shooting
...
between the closest shooting enemy stand and the closest
part of the unit as shown in <<drive-back>>. This is
similar to an evade move as described in the xref::movement.adoc#the-evade[Movement section],
but note that in the case of an evade,
the unit can change its formation and stands can change
their orientation as they move, in the case of a drive back,
a unit can do neither of these things but must move back
...
// Caption added.
.Driving back enemies
image::shooting/drive-back.svg[width=1312,height=1428,id=drive-back]
...
to move in their following [.edit]#Command & Initiative phase#.

When the shooter rolls for drive back distances, any dice
roll of a 6 means that the unit becomes *confused*.
...
image::shooting/chariot.webp[width=910,height=1088,role=decorative]
...
////
// WMR original removed:
// and which
// are not already engaged in combat
//
// This detail is easy to overlook, so replaced with the
// explicit paragraph from Bo5A below.
////
...
// Newline added,
// two sentences swapped (like Bo5A) to give more general rule first
...
xref:armies:goblins.adoc#wolf-riders[Wolf Riders]) and they can therefore shoot by drawing a
line of sight from any edge – other troops must draw a
line of sight from their front edge as usual.

// "Bear in mind that" removed
...
over into the combat [.bo5a]#phase#. When it comes to working out
...
image::shooting/shooting-at-chargers.webp[width=984,height=693,id=shooting-at-chargers]

If a missile-armed unit is charged by two or more enemy
units, one after the other, then it can potentially shoot
more than once. When shooting at a second or subsequent
charger only those stands that are not already touching
the enemy can fire. Remember that stands touching
corner-to-corner are not free to shoot
at charging enemy.
...
image::shooting/shooting-hits-carried-over.webp[width=984,height=676,id=shooting-hits-carried-over]

image::shooting/skull.webp[width=404,height=408,role=circle]
...
// Edit grammar
When a unit hit by shooting or magic [.edit]#is# already
engaged in combat, [.edit]#or becomes#
...
remove a stand and any remaining hits are always discounted at
...
image::shooting/charging.webp[width=2075,height=861,role=column-span]
