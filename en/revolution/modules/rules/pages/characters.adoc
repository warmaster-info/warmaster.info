...
single stand which includes the [.bo5a]#famous individual#
plus [.edit]#their# bodyguard, standard bearer, familiars, servants,
messengers, observers, courtiers, pets, jesters, toadies
and other such hangers-on as make up [.edit]#their# immediate
...
|===
|image:characters/general.webp[width=534,height=434]
|image:characters/wizard-1.webp[width=746,height=569]

|General |Wizard

|image:characters/hero.webp[width=514,height=597]
|image:characters/wizard-2.webp[width=541,height=590]
...
image::characters/character.webp[width=465,height=499]
...
image::characters/dryads.webp[width=2004,height=410,role=column-span]
...
as <<enemy-moving-through-characters,described below>>.
...
as <<characters-in-combat,described late>>r. A character stand on its own cannot
...
image::characters/dark-knight.webp[width=452,height=505,role=decorative]
...
the Command phase.
[.bo5a]#This section summarises the principle rules – for
details see the rules for xref:command.adoc#orders[Orders].#
...
orders until they too fail to issue an order.
...
image::characters/wolf-rider.webp[width=920,height=1330,role=decorative]
...
<<d6-blunder,Blunder table>>. The result applies to the unit which has
...
A General doesn’t blunder – [.bo5a]#they are above such things.#
However, if the General rolls
...
penalty for the rest of the battle.
...
to the brigade that includes the affected unit.
...
If there are visible enemy
units within a full pace move of the blundering unit
or brigade then the unit or each unit in a brigade
must move [.edit]#by# the shortest route to get [.edit]#beyond# a full
pace move from all enemy units.
...
[.edit]#by blocking line of# sight or access to [.edit]#stand# edges.
...
Once units have moved, or [.edit]#if# there is no visible
enemy, they halt. The unit/brigade cannot receive
any further orders that Command phase.
--

image::characters/night-goblins.webp[width=2147,height=281,role=column-span]
...
movement in the Command section. To summarise:
...
[.bo5a]#treat# terrain in the same way as infantry
[.bo5a]#regardless of whether the actual model is mounted.#
Only terrain
...
in the same way as these troops. See [.edit]#<<character-mounts>>#.

image::characters/tusks.webp[width=345,height=349,role=circle]
...
move as <<enemy-moving-through-characters,described below>>.
...
Command phase. Nominate a stand in the unit
[.edit]#measure# the distance to the commanded
...
[.bo5a]
All characters have an Attack bonus expressed as +1, +2 or +3.
If a character can shoot, it has an Attack bonus expressed as +1/1,
with the second value being the additional shooting attack.

[.bo5a]
A character that has a shooting attack can only use it when joined
to a unit.

////
// BO5A addition:
If several characters have joined a unit, only one
can use its shooting attack.
////
...
All shots taken at the same enemy unit have to
...
image::characters/ogres.webp[width=1695,height=621,role=column-span]
