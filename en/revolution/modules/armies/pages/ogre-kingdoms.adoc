...
to hand, and even using the machineries of war they take, trade
...
[[yhetees,†3]]3. Yhetees:: If a unit of Yhetees charges against an enemy
...
This spell can only be cast on an unengaged friendly unit within
range regardless of whether the Butcher can see it or not.

The target friendly unit can’t be driven back by enemy shooting
attacks until your next turn. In addition the unit cannot become
confused for any reason until the end of your next command
phase.
...
This spell can be cast on an enemy unit. The Butcher must
be able to see his target to use this spell. The spell cannot be
directed at a unit engaged in combat.

Bone Cruncher is treated like three ordinary shooting attacks
except that armour has no effect (all targets count as having no
armour). A unit can be driven back by Bone Cruncher as with
ordinary shooting.
...
This spell can be cast if the Butcher has joined a unit in combat.
...
the unit, adds +1 to its Attacks value. As the Butcher must
join the unit to cast the spell this includes the Butcher himself,
but it is possible for other characters to join the unit as well, in
...
This spell can be cast on one friendly unit within range that is
engaged in combat.

The affected unit increases its Hits value by +1 until the end
of the following Combat phase. At the end of the following
Combat phase, the unit’s Hits value returns to normal so any
outstanding Hits will be applied, potentially causing the loss
of stands.
